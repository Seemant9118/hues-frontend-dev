...
test('Can Edit Vendor', async ({ page }) => {
  ...
  await expect(page.getByRole('cell', { name: newUniqueName, exact: true })).toBeVisible();
});
...
test('Can Upload Vendor File', async ({ page }) => {
  ...
  await fileInput.setInputFiles('./__tests__/e2e/vendors/downloads/vendorSample.xlsx');
  await page.waitForTimeout(2000);
