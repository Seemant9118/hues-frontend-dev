...
export function GetEnterpriseUser(enterprise_id) {
    return APIinstance.get(enterprise_user.getEnterpriseUser.endpoint + `${enterprise_id}`);
}
...
export function DeleteEnterpriseUser(enterprise_id) {
    return APIinstance.delete(enterprise_user.deleteEnterpriseUser.endpoint + `${enterprise_id}`);
}