                ...
                <ul className="text-sm flex flex-col gap-2 px-6 text-neutral-500">
                    {
                        subItems?.map((subItem) => (
                            <li key={subItem.id} className="flex gap-2 items-center"><span>{subItem.icon ? subItem.icon : <Orbit size={10} />}</span><span className="text-neutral-500 font-semibold">{subItem.subItemtitle}</span> {subItem.item}</li>
                        ))
                    }
                </ul>
            </div>
