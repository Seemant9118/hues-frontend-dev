...
export function usePurchaseInvoiceFormConfig({ enterpriseId, setOrder }) {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      const moduleName = 'PURCHASE_B2BINVOICE';
      const config = await getFormConfig(moduleName);

      let fetchedFields = config.fields || [];
      if (fetchedFields.length === 0) {
        fetchedFields = DEFAULT_PURCHASE_FIELDS;
      } else {
        const fetchedKeys = fetchedFields.map((f) => f.key);
        const missingDefaults = DEFAULT_PURCHASE_FIELDS.filter(
