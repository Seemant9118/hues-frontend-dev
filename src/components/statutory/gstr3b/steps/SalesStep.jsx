...
export default function SalesStep({ formData }) {
  ...
  const {
    ...
    queryKey: [gstAPIs.syncInvoicesWithGSTR1.endpointKey, formData.period],
    queryFn: () => syncInvoicesWithGSTR1(formData.period),
    ...
    select: (res) => {
      ...
      // Portal invoices (Portal Only or Matched/Partial)
      ...
      matchedInvoices.forEach((m) => {
        const alreadyAdded = processed.some(
          (p) => p.docNo === m.portalInvoiceNumber && p.counterparty === m.ctin,
        );
        if (!alreadyAdded) {
          processed.push({
    ...
    <div className="mx-auto max-w-6xl space-y-2">
      <div>
        <h2 className="flex items-center gap-2 text-xl font-semibold">
          ...
          Sales vs Filed GSTR-1
        </h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Compare books sales invoices against filed GSTR-1.
        </p>
        <p className="mt-1 text-xs text-muted-foreground">
          Books as-of: {new Date().toLocaleString()} • GSTR-1 as-of:{' '}
          {new Date().toLocaleString()}
        </p>
      </div>

      <InfoBanner
        text="3B shows read-only diffs. To fix, use the Resolve button to open Sales Invoice Details."
          ...
          <TableBody>
            {isLoading ? (
                ...
                <TableCell colSpan={7} className="h-32 text-center">
                  <div className="flex items-center justify-center gap-2">
                    <RefreshCw
                      ...
                      size={20}
                    />
                    <span>Fetching sales invoices...</span>
                  </div>
                </TableCell>
              </TableRow>
            ) : isError ? (
              <TableRow>
                ...
                >
                  {error?.response?.data?.error === 'RET11407' ? (
                    ...
                    'Failed to fetch sales invoices. Please try again.'
                  )}
                </TableCell>
              </TableRow>
              ...
              salesInvoices.map((row) => (
                <TableRow
                  key={`${row.docNo}-${row.counterparty}-${row.status}`}
                >
                  <TableCell>
                    <Badge
                      variant="secondary"
                  ...
                  <TableCell>
                    <div className="flex items-center justify-center gap-2">
                      <Button
                        ...
                        onClick={() =>
                          router.push(
                            `/dashboard/sales/sales-invoices/${row.systemInvoiceNumber}`,
                          )
                        }
