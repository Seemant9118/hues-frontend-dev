...
export function useAgreementShare(agreement) {
  ...
  const createShareMutation = useMutation({
    mutationFn: async () => {
      const targetId = agreement?.id || agreement?.agreementId;
      if (!targetId) {
        throw new Error('Agreement template ID is missing.');
      }
    ...
    onSuccess: (data) => {
      setActiveShare(data);
      toast.success(
        `${sharingType === 'PUBLIC' ? 'Public' : 'Private'} agreement share link created successfully!`,
      );
    },
    onError: (err) => {
      toast.error(err.message || 'Error creating agreement share link.');
    },
  });
